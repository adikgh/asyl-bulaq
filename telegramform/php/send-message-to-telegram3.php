...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name3']) && !empty($_POST['phone3'])) {
    ...
    if (isset($_POST['name3']) && !empty($_POST['name3'])) {
        $txt .= "Имя пославшего: " . strip_tags(trim(urlencode($_POST['name3']))) . "%0A";
    }
    
    // Номер телефона
    if (isset($_POST['phone3']) && !empty($_POST['phone3'])) {
        $txt .= "Телефон: " . strip_tags(trim(urlencode($_POST['phone3']))) . "%0A";
    }
